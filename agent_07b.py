...
def send_email(recipient_email, subject, body):
    try:
        yag = yagmail.SMTP(
            ...
            # oauth2_file="oauth.json" # comment this line to text exception
        )
...
while user_input != "exit":
    ...
    history += response.output     # add output to history, even if it's a tool call
    ...
    for item in response.output:
        if item.type == "function_call":
            ...
            if function_name == "send_email":
                ...
                history += [{"type": "function_call_output", # add tool result to history
